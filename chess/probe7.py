...
def create():
	...
	figures = {26:(1,-4),30:(5,-50),33:(8,-4),110:(-1,4),114:(-5,50),117:(-8,4),113:(-6,4)}
...
def valid(typ,b,b_fig):
	...
	for m in moves:
		...
		if moves[m][1] in lon:
			...
			l = []	
	#print valid,'\n'
	# missing: rochade, schach/schach matt, bauer anfang
	return valid				
...
def ranking(md):
	...
	def tree(a_b,a_b_fig,md):
	
		global depth, xio, last
		
		if depth % 2 == 0:
			mo = valid('f',a_b,a_b_fig)
		else:
			mo = valid('e',a_b,a_b_fig)
		...
		for m in mo:
			for mov in mo[m]:
				...
				value[(x,depth)] = (check(b[m][0],mov,b[m][1],b,b_fig))
...
def main():
	...
	while True:	
		me_valid = valid('e',board,board_fig)
